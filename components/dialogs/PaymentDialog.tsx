...
/**
 * PaymentDialog component handles payment processing for a specific order.
 * It supports various payment methods including cash, transfer, QRIS, and card.
 */
function PaymentDialog({ open, onOpenChange, order, onSuccess }: PaymentDialogProps) {
  const [loading, setLoading] = useState(false);
  const remaining = Number(order?.totalPrice || 0) - Number(order?.totalPaid || 0);
  ...
  useEffect(() => {
    if (open && order) {
      const remainingVal = Number(order.totalPrice || 0) - Number(order.totalPaid || 0);
  ...
  async function loadBanks() {
    ...
    if (res.success && res.data) {
      setBanks(res.data.filter((b: BankAccount) => b.isActive));
    }
  }
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[500px] max-h-[90vh] flex flex-col p-6 overflow-hidden">
        <DialogHeader className="shrink-0">
          ...
          <DialogDescription>
            Input pembayaran untuk Order #{order?.id ? order.id.slice(-6) : ""} a.n{" "}
            {order?.custName || ""}
          </DialogDescription>
        </DialogHeader>

        <div className="shrink-0 bg-muted/40 border border-border p-4 rounded-lg space-y-2 mb-3">
          ...
          <div className="flex justify-between text-sm">
            ...
            <span className="font-semibold text-green-600 dark:text-green-400">
              Rp {Number(order?.totalPaid || 0).toLocaleString("id-ID")}
            </span>
          </div>
                ...
                <div className="grid grid-cols-2 gap-4 border border-border p-3 rounded-lg bg-muted/20">
                  ...
                  <div className="space-y-2 flex flex-col justify-end">
                    ...
                    <span className="text-lg font-bold text-green-600 dark:text-green-400">
                      ...
                      {Math.max(
                        0,
                        (Number(cashReceived) || 0) - (Number(amount) || 0)
                ...
                <div className="space-y-3 border border-border p-3 rounded-lg bg-muted/20">
                  ...
                  {method === "QRIS" && (
                    <div className="flex flex-col items-center justify-center p-4 border border-dashed border-border rounded bg-background/50">
                      <div className="w-32 h-32 bg-foreground/10 border-4 border-foreground/20 rounded flex items-center justify-center p-2 mb-2 relative">
                        ...
                        <span className="absolute text-[8px] bg-background px-1 py-0.5 rounded font-black border border-border">
                          QRIS MOCK
                        </span>
                      </div>
                      <p className="text-[10px] text-muted-foreground text-center font-medium">
                        Scan QR Code di atas menggunakan dompet digital atau aplikasi mobile banking
                        pelanggan.
                      </p>
                    </div>
                  )}
                  ...
                  {method === "CARD" && (
                    <div className="p-3 bg-background/50 border border-border rounded flex items-center gap-3">
                      <div className="w-8 h-8 rounded bg-primary/10 flex items-center justify-center font-bold text-xs text-primary border border-primary/20">
                        EDC
                      </div>
                      <div className="text-[10px] text-muted-foreground leading-relaxed">
                        Silakan swipe / insert kartu debit atau kredit pelanggan pada mesin EDC
                        tujuan. Klik <b>Bayar & Selesaikan</b> untuk mengonfirmasi struk EDC
                        berhasil keluar.
                      </div>
                    </div>
                  )}
